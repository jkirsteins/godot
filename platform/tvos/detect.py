...
def get_opts():
    ...
    return [
        ...
        ("TVOS_SDK_PATH", "Path to the tvOS SDK", ""),
        BoolVariable("tvos_simulator", "Build for tvOS Simulator", False),
        ("tvos_triple", "Triple for tvos toolchain", ""),
        BoolVariable("generate_bundle", "Generate an APP bundle after building tvOS binaries", False),
    ]
...
def configure(env: "SConsEnvironment"):
    ...
    if env["tvos_simulator"]:
        ...
        env.Append(ASFLAGS=["-mappletvsimulator-version-min=16.0"])
        env.Append(CCFLAGS=["-mappletvsimulator-version-min=16.0"])
        env.Append(CPPDEFINES=["TVOS_SIMULATOR"])
        env.extra_suffix = ".simulator" + env.extra_suffix
    else:
        detect_darwin_sdk_path("tvos", env)
        env.Append(ASFLAGS=["-mappletvos-version-min=16.0"])
        env.Append(CCFLAGS=["-mappletvos-version-min=16.0"])

    if env["arch"] == "x86_64":
        if not env["tvos_simulator"]:
            print_error("Building for tvos with 'arch=x86_64' requires 'tvos_simulator=yes'.")
            sys.exit(255)
        ...
        env.Append(
            CCFLAGS=(
                ...
                " -fasm-blocks -isysroot $TVOS_SDK_PATH"
            ).split()
        )
    ...
    if env["metal"]:
        ...
        env.Prepend(
            CPPPATH=[
                "$TVOS_SDK_PATH/System/Library/Frameworks/Metal.framework/Headers",
                "$TVOS_SDK_PATH/System/Library/Frameworks/QuartzCore.framework/Headers",
            ]
        )
    ...
    if env["opengl3"]:
        ...
        env.Prepend(
            CPPPATH=[
                "$TVOS_SDK_PATH/System/Library/Frameworks/OpenGLES.framework/Headers",
            ]
        )
